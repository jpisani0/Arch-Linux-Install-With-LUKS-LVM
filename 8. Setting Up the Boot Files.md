...
The important hooks to add, as most of these will probably be there already, are the `sd-encrypt` and `lvm2` hooks in between the `block` and `filesystems` hooks. These are needed to decrypt our LUKS partitions at boot and have the LVM setup our volume group . After you save and close the file, we will also need to install the `lvm2` package with:

```bash
pacman -S lvm2
```

We will also edit the `/etc/crypttab` file and add our encrypted devices. We will be adding the devices by UUID to ensure we are targeting the correct devices at boot. To get the UUIDs of out encrypted devices, we can use:

```bash
blkid /dev/<your-encrypted-device>
```

Do this for each of your encrypted devices. After, we can edit the `/etc/crypttab` file and add a line for each device at the end of the file like this:

```bash
<mount-name> UUID=<UUID> none luks
```

For example, my two encrypted devices are `/dev/nvme0n1p2` and `/dev/nvme1n1p1` and I want to have them mounted as `luks_lvm0` and `luks_lvm1` respectively, so I will do:

```bash
luks_lvm0 UUID=</dev/nvme0n1p2-UUID> none luks
luks_lvm1 UUID=</dev/nvme1n1p1-UUID> none luks
```

Make sure that the mount name is the same that you gave these devices during the encryption step.
...
You may see some warnings but those can be safely ignored. If you get an error for the `sd-vconsole` hook, you can get rid of it by adding a file at `/etc/vconsole.conf` and writing `KEYMAP=us` (or your own keymap) into it.
